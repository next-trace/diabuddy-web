...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const { response, rotatedTokens } = await authedBackendFetch(req, '/admin/ai/subagents', { method: 'GET' });
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  ...
  const { response, rotatedTokens } = await authedBackendFetch(req, '/admin/ai/subagents', {
    method: 'PUT',
